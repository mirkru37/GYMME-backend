  ...
  module Contracts
    ...
    class Update < ApplicationContract
      ...
      params do
        ...
        optional(:primary_muscle_id).maybe(:string)
        optional(:secondary_muscle_ids).maybe(:array)
      end

      rule(:name) do
        key.failure('must be less than 30 characters') if value && value.length > NAME_MAX_LENGTH
      end

      rule(:instructions) do
        key.failure('must be less than 2000 characters') if value && value.length > INSTRUCTIONS_MAX_LENGTH
      end

      rule(:primary_muscle_id) do
        key.failure('must exist') if value && !Muscle.exists?(id: value)
      end
